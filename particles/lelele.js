...
class leleleParticle extends Particle {
    ...
    display() {
      ...
      let transitionRatio = (currentTime - this.transitionStart) / this.transitionDuration;
      let lerpedColorA = lerpColor(this.colorA, this.colorB, min(transitionRatio, 1)); // Farbe für den ersten Kreis
      let lerpedColorB = lerpColor(this.colorB, this.colorA, min(transitionRatio, 1)); // Farbe für den zweiten Kreis
  
      // Erste Ellipse mit radialen Verlauf
      noStroke();
      for (let i = 0; i < this.gradientRadius; i++) {
        let lerpedColor = lerpColor(lerpedColorA, color(lerpedColorA.levels[0], lerpedColorA.levels[1], lerpedColorA.levels[2], 0), i / this.gradientRadius);
        fill(lerpedColor);
      ...
      for (let i = 0; i < this.gradientRadius; i++) {
        let lerpedColor = lerpColor(lerpedColorB, color(lerpedColorB.levels[0], lerpedColorB.levels[1], lerpedColorB.levels[2], 0), i / this.gradientRadius);
        fill(lerpedColor);
